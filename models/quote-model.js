...
async function checkResponse(quote_id) {
    try {
...
async function viewQuotes() {
    try {
        const result = await pool.query("SELECT * FROM quotes WHERE quote_offer_price = NULL");
        return result.rows
    } catch (error) {
        return error.message;
    }
...
async function processQuote(quote_id, quote_asking_price) {
    try {
        const sql = "UPDATE quotes SET quote_offer_price = $1 WHERE quote_id = $2 RETURNING *";
        const result = await pool.query(sql, [quote_asking_price, quote_id]);
        return result.rows[0]
    } catch (error) {
        return error.message;
    }
...
module.exports = { newQuote, checkResponse, changeQuote, viewQuotes, processQuote };